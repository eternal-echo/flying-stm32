...
void printPaddedInt16b(int16_t val);              // 打印一个格式化的16位整数
void printRawAGMT(ICM_20948_AGMT_t agmt);         // 打印原始的加速度、陀螺仪、磁力计和温度数据
void printFormattedFloat(float val, uint8_t leading, uint8_t decimals); // 打印格式化的浮点数
#ifdef USE_SPI
void printScaledAGMT(ICM_20948_SPI *sensor);      // 打印按比例缩放的传感器数据（使用SPI时）
#else
void printScaledAGMT(ICM_20948_I2C *sensor);      // 打印按比例缩放的传感器数据（使用I2C时）
#endif
